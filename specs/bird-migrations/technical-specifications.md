...
- **Proportions de la zone de montage :** le rectangle géographique ci-dessus est nettement plus haut que large (ratio largeur/hauteur ≈ 0,59 sous cette projection), mais la hauteur de la zone de montage suit `max(480px, largeur × 0,85) / 1,5` (soit `max(320px, largeur × 0,567)`), pas un ajustement pixel-perfect à ce ratio (qui demanderait une hauteur ≈ 1,7 fois la largeur, disproportionnée dans la mise en page du site) : compromis en faveur d'une carte courte plutôt que d'un cadrage large-écran ou de la fidélité totale au ratio géographique, au prix d'une bande de mer/désert supplémentaire visible de part et d'autre de l'Europe et l'Afrique. Le diviseur 1,5 est un réglage manuel (demande explicite), pas dérivé d'un calcul géométrique.
