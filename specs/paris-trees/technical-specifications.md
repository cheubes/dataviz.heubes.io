...
- **Hauteur de la zone de carte :** hauteur par défaut du site (voir "Règles communes à toutes les visualisations" dans `technical-specifications.md` général), pas un ajustement au ratio largeur/hauteur géographique réel de Paris (≈ 1,87, dérivé de `geoBounds` sur `districts.json`, corrigé de la latitude).
