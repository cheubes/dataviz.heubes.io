...
**Éruptions hors catalogue de volcans holocènes :** 41 éruptions sur les 10 978 restantes (vingt `Volcano_Number` distincts, ex. Nevsehir-Acigol Volcanic Complex, Tres Virgenes) référencent un volcan absent de la couche `Holocene_Volcanoes` récupérée. Vérifié à l'investigation : ces vingt volcans existent bien au GVP, mais classés dans la couche `Smithsonian_VOTW_Pleistocene_Volcanoes` plutôt que `Holocene_Volcanoes` — une incohérence de classification propre au GVP (l'édifice volcanique est catalogué "pléistocène" alors qu'une éruption individuelle qui lui est rattachée porte une date holocène), pas un bug de récupération de notre script. Exclues du jeu de données généré au même titre que les éruptions antérieures à -8000 ci-dessus (le catalogue de volcans de cette visualisation est scopé à `Holocene_Volcanoes` spécifiquement, voir "Récupération des données" ci-dessus) : **10 937 éruptions** au total dans `eruptions.json`. Aucune des 41 n'atteint le seuil VEI ≥ 6 (voir "Sélection des éruptions à annoter" ci-dessous), donc aucun impact sur les éruptions notables.

**Volcans sans éruption datée dans le catalogue (27 %) :** 333 volcans sur 1 214 n'ont aucune éruption dans `eruptions.json` (queue de la distribution : la plupart faute d'entrée dans la couche `Holocene_Eruptions`, identifiés par indice géologique sans éruption individuellement datable, voir `Evidence_Category` du GVP). Conservés dans `volcanoes` (point de repos permanent affiché, voir "Volcans au repos" dans `technical-specifications.md`) mais ne pulsent jamais sur le cycle.
