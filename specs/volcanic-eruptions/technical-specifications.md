...
**Canvas 2D**, même choix que `bird-migrations`/`earthquakes`/`satellites-in-orbit`/`monument-layers` : 10 978 éruptions (11 089 dans le catalogue Holocène brut, 111 antérieures à -8000 exclues) et 1 214 volcans (voir "Récupération des données" et "Champs source utilisés" dans `data-model.md`), du même ordre de grandeur que `earthquakes` (14 453 séismes).
