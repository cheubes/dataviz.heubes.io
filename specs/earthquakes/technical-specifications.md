...
- **Réutilisation des assets de `bird-migrations` :** copiés dans `public/data/earthquakes/` plutôt que référencés depuis le dossier de `bird-migrations` (voir "Techno carte" ci-dessus), tranche la question restée ouverte dans `animal-migrations`/`volcanic-eruptions`, au moins pour cette visualisation.
