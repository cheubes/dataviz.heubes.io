...
- Titre `h1` "À propos" / "About", puis un texte en sections `h2` :
  ...
  - **Fabrication** : site statique, visualisations codées une par une, hébergement, code source public ; pas de mesure d'audience ni de cookie, seule la préférence de langue est mémorisée dans le navigateur (voir "Détection et mémorisation de la langue" dans `technical-specifications.md`).
