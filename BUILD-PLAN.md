...
**Statut :** fait, neuf visualisations réelles à ce stade :
...
- `animal-migrations` (Les grandes migrations), un seul planisphère mondial réunissant trois mécaniques de migration radicalement différentes (voir `specs/animal-migrations/data-model.md`) : trajectoires GPS/Argos individuelles réelles de baleines à bosse et de baleines bleues (quatre individus chacune, Movebank Data Repository, accessible en lecture anonyme via son API REST malgré l'authentification requise par l'API de tracking principale), trajectoire individuelle en boucle de trois gnous à barbe blanche de l'écosystème Mara (même source, fenêtre d'un an extraite par individu plutôt que le déploiement complet), et densité d'occurrences GBIF du papillon monarque agrégée sur une grille régulière de 2° en Amérique du Nord (570 cellules, méthode adaptée en cours d'implémentation : facettes mensuelles de l'API de recherche publique GBIF plutôt que l'API SQL Downloads, faute de compte GBIF disponible). Rendu Canvas 2D sur le même fond de carte que `bird-migrations` (silhouette, relief et cours d'eau mondiaux réutilisés tels quels, seul le `fitExtent` change), trois canvas superposés plutôt que deux (la densité, redessinée intégralement à chaque frame, ne peut pas partager son calque avec l'effet de traînée des trajectoires, qui repose au contraire sur un calque jamais vidé), pas de notion de direction automne/printemps (ne généralise pas à un panorama mondial, à la différence de `bird-migrations`) ni de bascule vue animée/statique, filtre par espèce, couverture en placeholder partagé en attendant une image propre.
